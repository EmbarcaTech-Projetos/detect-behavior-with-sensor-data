...
int mlx90614_read_raw_data(uint8_t read_buffer[3]) {
    ...
    i2c_write_blocking(MLX90614_I2C_PORT, MLX90614_ADDR, &MLX90614_REG_TOBJ1, 1, true);
        
    // Read 3 bytes: Temperature LSB, Temperature MSB, and PEC (Packet Error Code)
    int result = i2c_read_blocking(MLX90614_I2C_PORT, MLX90614_ADDR, read_buffer, 3, false);
    return result; 
}